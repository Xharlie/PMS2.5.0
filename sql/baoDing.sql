/*
 ...
 Date: 03/17/2015 00:25:10 AM
*/

CREATE DATABASE IF NOT EXIST BI;
USE BI;
SET NAMES utf8;
...
CREATE TABLE `MemberInfo` (
  ...
  `IN_TSTMP` datetime DEFAULT NULL COMMENT 'become member dateTime',
  ...
  PRIMARY KEY (`MEM_ID`)
) ENGINE=InnoDB AUTO_INCREMENT=9291226 DEFAULT CHARSET=utf8 ROW_FORMAT=COMPACT;
...
CREATE TABLE `RoomDepositAcct` (
  ...
  `FILLED` varchar(2) DEFAULT NULL COMMENT 'F for not been checked out, T means been checked out',
...
CREATE TABLE `Shifts` (
  ...
  `SHFT_ST_CSH` float(12,2) DEFAULT NULL,
  `SHFT_ST_TSTMP` datetime DEFAULT NULL,
  `SHFT_PSS_EMP_ID` varchar(15) DEFAULT NULL COMMENT 'employee id of who passing shift',
  `SHFT_END_TSTMP` datetime DEFAULT NULL,
  `SHFT_END_PSS2_CSH` float(12,2) DEFAULT NULL COMMENT 'shift end cash',
  `SHFT_CSH_AID` float(12,2) DEFAULT NULL COMMENT 'shift cash aided by accountant(when not enough cash left )',
  `SHFT_CSH_BOX` float(12,2) DEFAULT NULL COMMENT 'how much the shift deposit the extra cash to the profit box when shift ends',
  PRIMARY KEY (`SHFT_ID`)
...
-- ----------------------------
--  Event structure for `EVT_NIGHT_CHECK`
-- ----------------------------
DROP EVENT IF EXISTS `EVT_NIGHT_CHECK`;
delimiter ;;
CREATE DEFINER=`root`@`localhost` EVENT `EVT_NIGHT_CHECK` ON SCHEDULE EVERY 1 DAY STARTS '2015-03-17 03:00:00' ON COMPLETION PRESERVE ENABLE DO BEGIN
